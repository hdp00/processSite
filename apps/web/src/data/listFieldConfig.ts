...
export const DEFAULT_SYSTEM_LIST_FIELDS: SystemListFieldConfig[] = [
  ...
  { key: "template", label: "流程名称", description: "实例所属的流程定义名称", taskVisible: true, processListVisible: true, exportVisible: true },
  { key: "templateVersion", label: "版本", description: "实例发起时固定使用的流程版本", taskVisible: true, processListVisible: true, exportVisible: true },
  { key: "status", label: "状态", description: "审核中、已完成、已关闭等运行状态", taskVisible: false, processListVisible: true, exportVisible: true },
  { key: "currentNode", label: "当前节点", description: "当前正在处理的流程节点或受理人", taskVisible: true, processListVisible: true, exportVisible: true },
  { key: "round", label: "当前轮次", description: "驳回后重新提交产生的审核轮次", taskVisible: true, processListVisible: true, exportVisible: true },
  { key: "initiator", label: "发起人", description: "创建该流程实例的用户", taskVisible: true, processListVisible: true, exportVisible: true },
  { key: "createdAt", label: "发起时间", description: "流程实例首次创建的时间", taskVisible: false, processListVisible: true, exportVisible: true },
  { key: "updatedAt", label: "更新时间", description: "流程实例最近一次发生变化的时间", taskVisible: false, processListVisible: true, exportVisible: true },
];
...
export function loadSystemListFields(definitionId: string): SystemListFieldConfig[] {
  ...
  try {
    ...
    return DEFAULT_SYSTEM_LIST_FIELDS.map((fallback) => {
      const saved = parsed.find((item) => item.key === fallback.key);
      return {
        ...fallback,
        ...saved,
        key: fallback.key,
        ...
        exportVisible: saved?.exportVisible ?? saved?.processListVisible ?? fallback.processListVisible,
      };
    });
  } catch {
