    ...
    }),
    {
      ...
      migrate: (persisted) => {
        ...
        const definitions = legacyState.definitions.map((raw) => {
          ...
          const allocated = versions.map((version) => Number(version.version.replace(/\D/g, ""))).filter(Number.isFinite);
          return { id: String(raw.id), code: String(raw.code ?? published?.basic.code ?? versions[0]?.basic.code ?? "PROC-UNKNOWN"), name: published?.basic.name ?? String(raw.name ?? versions[0]?.basic.name ?? "未命名流程"), description: published?.basic.description ?? String(raw.description ?? versions[0]?.basic.description ?? ""), type: (raw.type as DefinitionType) ?? versions[0]?.basic.type ?? "approval", disabled: Boolean(raw.disabled), publishedVersionId: versions.some((version) => version.id === publishedVersionId) ? publishedVersionId : undefined, nextVersionNumber: Number(raw.nextVersionNumber ?? Math.max(0, ...allocated) + 1), versions, updatedAt: String(raw.updatedAt ?? nowText()), updatedBy: String(raw.updatedBy ?? "历史数据"), instanceCount: Number(raw.instanceCount ?? versions.reduce((total, version) => total + version.instanceCount, 0)) } satisfies ProcessDefinition;
