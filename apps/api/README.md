...
普通用户域登录、用户/角色完整维护、部门/职务写操作、流程发布、实例详情和实例业务写操作仍需按纵向切片继续实现。代码默认保持简单：`Controller → Service → EF Core/少量参数化 SQL`，不预先增加 CQRS、通用仓储或事件总线。
...
- 可读取部门、职务、用户和角色目录，可创建用户与非内置角色；用户/角色详情、修改、状态、密码和删除尚未实现。
