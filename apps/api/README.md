...
普通用户域登录和实例后续流转操作仍需按纵向切片继续实现。代码默认保持简单：`Controller → Service → EF Core/少量参数化 SQL`，不预先增加 CQRS、通用仓储或事件总线。
...
- 发起前附件上传要求 `Idempotency-Key` 以及当前发布版本的 `definitionId/versionId/fieldId`。文件直接流式写入按年分层的 `.incoming`，同时校验 100 MB 系统上限、字段上限、扩展名、PDF/MZ 内容签名并计算 SHA-256；实例创建或首个审核结果前的内容保存事务再把当前用户的暂存附件转为有效引用。元数据和内容读取执行上传人或实例数据范围校验，内容接口支持单 Range；替换后不再引用的附件进入待清理状态。
