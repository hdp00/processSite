...
普通用户域登录、角色完整维护、部门/职务写操作、流程发布、实例详情和实例业务写操作仍需按纵向切片继续实现。代码默认保持简单：`Controller → Service → EF Core/少量参数化 SQL`，不预先增加 CQRS、通用仓储或事件总线。
...
- 角色目前支持目录、创建和权限矩阵维护；角色详情、修改和删除尚未实现。部门、职务目前只支持读取。
