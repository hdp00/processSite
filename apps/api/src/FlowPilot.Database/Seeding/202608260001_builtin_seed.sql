...
(
    ...
    WHERE [normalized_code] = @system_department_code AND [id] <> @system_department_id
)
    THROW 51110, 'Built-in department conflicts with an existing row.', 1;

UPDATE [flowpilot].[departments]
SET [code] = @system_department_code,
    [normalized_code] = @system_department_code,
    [name] = @system_department_name,
    [parent_id] = NULL,
    [path_cache] = @system_department_name,
    [sort_order] = 0,
    [is_enabled] = 1,
    [description] = N'FlowPilot 系统内置部门',
    [revision] = [revision] + 1,
    [updated_at] = @now
WHERE [id] = @system_department_id
  AND
  (
      [code] <> @system_department_code OR [normalized_code] <> @system_department_code
      OR [name] <> @system_department_name OR [parent_id] IS NOT NULL
      OR [path_cache] <> @system_department_name OR [sort_order] <> 0
      OR [is_enabled] <> 1 OR ISNULL([description], N'') <> N'FlowPilot 系统内置部门'
  );
SET @changed += @@ROWCOUNT;

IF NOT EXISTS
(
    SELECT 1 FROM [flowpilot].[departments] WITH (UPDLOCK, HOLDLOCK)
    WHERE [id] = @system_department_id
)
BEGIN
    INSERT INTO [flowpilot].[departments]
    (
        [id], [code], [normalized_code], [name], [parent_id], [path_cache],
        [sort_order], [is_enabled], [description], [revision], [created_at], [updated_at],
        [created_by], [updated_by]
    )
    VALUES
    (
        @system_department_id, @system_department_code, @system_department_code,
        @system_department_name, NULL, @system_department_name,
        0, 1, N'FlowPilot 系统内置部门', 1, @now, @now, NULL, NULL
    );
    SET @changed += 1;
END;
...
VALUES
    (@system_position_id, @system_position_code, @system_position_name, 0, N'FlowPilot 系统内置职务'),
...
SET [login_name] = @super_admin_login_name,
    ...
    [position_id] = @system_position_id,
    [is_enabled] = 1,
...
WHERE [id] = @super_admin_user_id
  ...
  (
      ...
      OR [authentication_mode] <> N'password' OR [department_id] <> @system_department_id
      OR [position_id] <> @system_position_id OR [is_enabled] <> 1
...
BEGIN
    ...
    (
        ...
        @system_department_id, @system_position_id, 1, 1, 1, @now, @now, NULL, NULL
    );
    SET @changed += 1;
END;
