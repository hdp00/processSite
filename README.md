...
完成初始化和 Seed 后，`/health/ready` 可验证数据库结构与种子版本；当前支持 `superadmin` 登录、会话恢复和注销，用户列表、详情、创建、编辑、启停、密码重置和受引用保护的删除，可读取部门、职务和角色、创建角色、维护角色权限与流程权限组，也可读取任务中心、“我的发起”和流程定义，创建定义、保存 V1 的基本信息/表单/流程图并重新校验。域登录、角色完整维护、部门/职务写操作、流程发布及实例业务写入尚未实现。
