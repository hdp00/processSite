...
> 整改复核（2026-08-20）：P0 三项已关闭。远程模式现在恢复 `/auth/me` 会话并一次性水合目录、定义、实例和当前用户任务；流程定义向导、发布、版本、审批、关闭、自由协作和附件写命令已通过 REST，响应只写入客户端缓存。附件采用暂存后随业务命令提交；完整发布校验成为唯一权威入口。富媒体已移出 localStorage，Excel 转 PDF 配置、双重校验、Mock 链路和 OpenAPI 已补齐。剩余工作仅包括正式 NestJS/LibreOffice 实现、治理目录写操作的 REST 页面迁移，以及浏览器 E2E；这些不能用当前纯前端仓库伪装成已完成后端。
...
因此，当前版本已经可以作为正式后端的前端契约基线，但仍不是可直接投产的完整系统：仓库中尚无 NestJS、SQLite/SQL Server、真实磁盘附件服务、LibreOffice 进程和 SMTP worker。上述服务端能力必须按 OpenAPI 与需求文档另行实现和部署，不能由浏览器 Mock 代替。
...
| --- | ---: | --- |
| P0 | 0 | 原 3 项均已关闭 |
| P1 | 2 | 正式后端/LibreOffice 尚不存在；治理目录页面写命令仍待随 NestJS 接口迁移 |
...
当前原型未实现已确认的 LibreOffice 转换链路；直接调用 API 还可以把不符合目标字段规则的暂存文件关联到实例。未来若照此契约实现后端，会形成真实上传安全风险。

**建议**

扩展附件字段快照与 OpenAPI：允许扩展名、Excel 转 PDF、最大页数。建立暂存附件票据并携带目标 `definitionId/versionId/fieldId`，上传和实例提交两次校验；服务端通过文件签名识别类型并统一拒绝危险文件。Excel 转换采用异步状态机和隔离临时目录，转换成功前实例命令拒绝提交。
...
2. 增加 Excel 转 PDF 配置、转换任务、最大页数和失败清理契约。
