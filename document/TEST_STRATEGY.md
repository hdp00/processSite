...
根目录的 `pnpm test:all` 先运行 `backend:check`。该步骤要求 OpenAPI 契约检查、后端 Release 构建、后端单元测试、HTTP/API 测试和真实 SQL Server 集成测试全部通过；随后才运行前端 `test:all`。不要把 `apps/web` 包内同名脚本误认为根目录完整门禁。
...
- Playwright 每个用例使用独立 BrowserContext，并连接真实后端。默认把 Debug API 和数据库工具构建到测试专用输出目录，再在独立的 `3100` 端口启动，避免占用或覆盖 `dotnet watch` 正在使用的文件。测试开始时从开发配置派生名称以 `_PlaywrightTests_<API 端口>` 结尾的专用数据库，重建结构并初始化内置数据；结束时即使用例失败也删除该数据库和测试附件目录。删除操作带严格名称校验，绝不删除开发数据库。四个 Chromium worker 并行执行也作为会话和基础目录并发读取的回归验证。至少一个冒烟用例禁用 localStorage、sessionStorage 和 IndexedDB，防止重新引入浏览器业务数据源。
...
- Playwright 默认启动本地 API 与 Vite，并使用自动创建和清理的隔离数据库与附件目录；本地配置的服务器、账号、密码、排序规则和超级管理员初始密码仍作为连接基础，但不会向开发数据库写入业务数据。也可通过 `FLOWPILOT_TEST_BASE_URL` 指向已部署服务；此模式不会创建或删除远端数据库，并自动跳过会写入流程业务数据的隔离链路。账号通过 `FLOWPILOT_E2E_USERNAME`、`FLOWPILOT_E2E_PASSWORD` 配置，本地未设置密码时可读取被 Git 忽略的后端开发配置。
...
- 自由协作测试必须验证唯一当前受理人与唯一 pending 任务一致，变更受理人、关闭和重开均原子更新任务、时间线、参与人投影与实例 `updated_at`；回复编辑只保留最新正文和编辑审计元数据，数据库与 API 均不得泄漏旧正文。
