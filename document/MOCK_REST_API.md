...
- 已有用户停用后可以先清空全部角色，再请求删除；新增或启用账号仍必须至少分配一个角色。权限矩阵使用 `flowpilot-role-permissions-v3` 定向持久化；从 v1/v2 升级时保留自定义角色权限，只为原本拥有全部既有权限的系统管理员补齐新增删除权限。角色列表中的页面权限和动作权限数量直接从该矩阵计算。
