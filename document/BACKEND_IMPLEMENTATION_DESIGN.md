...
旧 NestJS 骨架已删除。当前 `apps/api` 已完成 .NET 10 工程、健康检查、数据库初始化/Seed、超级管理员登录会话、用户与角色完整维护、组织目录读取、流程权限组管理、任务中心/流程实例列表，以及流程定义/版本读取、定义创建与 V1 基本信息/表单/流程图保存和重新校验切片；其他业务能力按本文件逐个实现，不提供占位成功接口。
