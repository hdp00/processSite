...
旧 NestJS 骨架已删除。当前 `apps/api` 已完成 .NET 10 工程、健康检查、数据库初始化/Seed、超级管理员登录会话、组织与权限完整维护、任务中心、流程定义和流程实例的创建/列表/详情，以及发起前附件暂存、元数据、暂存删除和受控内容读取切片；其他业务能力按本文件逐个实现，不提供占位成功接口。
