...
当前部署的 Windows Server 2016 和 SQL Server 2016 SP2 保持不变。本项目明确接受其生命周期风险和 SQL Server 连接不启用 TDS 传输加密的风险，不制定服务器或数据库升级计划；后端同时保持对之后 SQL Server 版本的兼容。仍须执行内网隔离、最小权限、数据库端口来源限制、适用补丁、备份恢复和审计等补偿性控制。
...
SQL Server 连接字符串必须显式给出传输加密、服务器证书信任、连接建立超时和连接池上下限，避免 Microsoft.Data.SqlClient 升级改变默认行为。当前已确认的隔离内网部署统一使用 `Encrypt=false;TrustServerCertificate=true`，生产启动校验和安装脚本均要求这组显式值；该连接不提供 TDS 传输加密，风险由部署方接受，并依赖内网隔离、最小权限和访问控制。常规 EF、就绪元数据/种子、结构探测、迁移预检和迁移 DDL 的命令超时统一从强类型 `FlowPilot:Database` 选项读取，使用有限的默认值并允许外置覆盖，禁止用 `0` 配置无限命令等待。连接字符串诊断只能输出脱敏结果。
...
- 当前隔离内网部署使用 `ldap://`，并要求外置配置显式设置 `AllowPlainText=true` 作为风险确认；实现仍兼容 `ldaps://`。域故障不得回退本地密码。
...
审批任务激活、流程完成、固定审批事项关闭、自由协作受理任务创建或自由协作关闭时，由业务命令在保存流程状态的同一事务中写入 Outbox。固定审批关闭始终通知发起人，不依赖节点邮件开关；自由协作使用版本基本信息中的流程级邮件开关统一控制受理与关闭通知，开启时每次创建新受理待办只通知该次操作后当前选中的最新受理人，关闭时通知发起人。提交后 `BackgroundService` 只领取已有消息并使用 MailKit 发送，不再扫描流程状态补建消息。后台邮件、附件清理、过期会话和幂等清理均通过 SQL 租约领取，进程重启后能恢复，不能只依赖内存定时器。
...
| 日期 | 变更内容 |
| --- | --- |
| 2026-09-02 | 固定审批新节点邮件默认开启，固定审批关闭始终通知发起人；自由协作版本增加默认开启的流程级邮件开关，并统一控制受理人与关闭发起人通知。 |
...
| 2026-08-28 | 完成首版 REST 契约收口：流程定义生命周期与导入导出、权限组变更影响、邮件 Outbox、审计、流程清单 Excel 数据集和详细运维状态均已落地；接入 LDAP/LDAPS、MailKit SMTP 及后台附件、会话、幂等和邮件清理。 |
