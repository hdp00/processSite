...
旧 NestJS 骨架已删除。当前 `apps/api` 已完成 .NET 10 工程、健康检查、数据库初始化/Seed、超级管理员登录会话与服务端模拟身份、组织与权限完整维护、任务中心、流程定义、流程实例创建/列表/详情、重新提交与关闭、自由协作回复、回复编辑、变更受理人、初始表单修改、关闭与重新打开，以及审批任务首次结果、重复字段修改和实例字段附件暂存切片；其他业务能力按本文件逐个实现，不提供占位成功接口。
...
自由协作实例在进行中保存唯一当前受理人，并与唯一待处理自由协作任务保持一致。回复和流转事件保存到专用时间线表；参与人是由发起人、历次受理人和回复作者事务内维护的可重建投影。回复编辑只覆盖原回复的最新正文，同时保存编辑人、编辑时间和一条通过 `related_entry_id` 指向原回复、但不含旧正文的编辑事件，不建立回复正文版本表。创建、回复、编辑、变更受理人、关闭、重开、任务变化、参与人投影、附件引用和实例 `updated_at` 必须在相应领域事务内原子提交。

当前已实现文字回复、回复编辑与变更受理人：回复不改变受理人；变更受理人由锁定版本发起权限组或审批/受理权限组的当前有效成员执行，不要求是实际发起人或当前受理人。该操作完成原待办并创建新受理人的唯一待办，也可在同一请求内先发表回复；不再设置独立的异常改派接口。新增回复和变更受理人使用实例 ETag 与幂等键，回复编辑按契约只使用实例 ETag；服务端原子更新参与人、时间线、审计与实例版本。编辑只保留最新正文，另存不含旧正文的编辑事件。已确认新增回复可以携带一个或多个暂存附件，并与回复正文、附件引用和时间线原子提交；该附件切片尚待实现，当前代码仍拒绝非空 `attachmentIds`，不得返回占位成功。
